...
const QuestionStatus = ({ questions, setCurrentQuestion }) => {
  const statusColors = {
    answered: "bg-[#A9E0B1] text-[#1C922F]",   // Green for answered
    visited: "bg-[#CDDBFF] text-[#2F84C3]",      // Blue for visited but not answered
    marked: "bg-[#F9F4E7] text-[#BE9939]",   // Yellow for marked for later
      ...
      <div className="flex flex-wrap gap-4">
        {section.map((question) => (
          <div
            ...
            className={`w-12 h-12 flex items-center justify-center rounded-full cursor-pointer ${statusColors[question.status]} text-sm font-semibold`}
          >
            {`Q${question.number}`}
          </div>
        ))}
